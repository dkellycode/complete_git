...
intypes<-c("S", "SLOB", "EOL")
iss<-iss%>%mutate(Instock = ifelse(iss$`Qty Avail` <= 0, "OUT", "IN"))%>%left_join(.,prodtype, by = "Product Code")
iss<-filter(iss, iss$`Product Type` %in% intypes)
...
#WHy does filter on 2 variable return zero results?
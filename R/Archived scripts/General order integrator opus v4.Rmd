...
sime<-sime<-list.files("\\\\CPSL2K12\\PropellaEDI\\live\\Orders to import",pattern = "^\\d{7}", full.names = TRUE)%>%lapply(.,read_csv, show_col_types = FALSE)%>%do.call(rbind,.)
...
checklist<-list(farm,aciem,smart,biw,pgg,isl,iml,sime,viridian)
names(checklist)<-c("importfarm","importaciem","importsmart","importbiw","importpgg","importisl","importiml","importsime","importviridian")
printlist<-names(compact(checklist))

filecount<-length(list.files("\\\\CPSL2K12\\PropellaEDI\\live\\Orders to import", full.names = TRUE)%>%.[!file.info(.)$isdir])

message(paste(filecount," orders read"))
```
...
importpgg['Freight Value']<-as.numeric(0)
```
...
importisl['Freight Value']<-as.numeric(0)
```
...
importsime['Freight Value']<-as.numeric(0)
```
...
importviridian['Freight Value']<-as.numeric(0)
```

```{r Combineprint}
#Join all the individual tables together and give the data a final clean (including freight calculation from Propella)
finalcheck<-c("importfarm","importaciem","importsmart","importbiw","importpgg","importisl","importiml","importsime","importviridian")
deletions<-c(setdiff(finalcheck,printlist))%>%rm(.)
importcombined<-printlist%>%reduce(full_join)
importcombined['Prod No']<-apply(importcombined[15],2, function (x) toupper(x))
importcombined<-left_join(importcombined,generalfreight, by = c("Prod No" = "Product Code"))%>%mutate(., 'Freight Total' = importcombined$QTY*importcombined$Freight.y)%>%.[c(1:12,21,14:16)]

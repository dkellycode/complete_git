...
if_else(is.null(nrow(farm)), message("No orders"),tablebuilder(impfarm))
```
...
if_else(is.null(nrow(aciem)), message("No orders"),tablebuilder(impaciem))
```
...
# Change the column headings to match XLSX import format
try(colnames(impsmart)<-c('CustOrd','Ref','OrdDate','Cust No','Addr1','Addr2','Addr3','Addr4','City','Pcode','Phone','Email','Freight Value','Stkpt','Prod No','QTY'))

#Hard code and clean
try({impsmart['Cust No']<-c(replicate(nrow(smart),"SMA001"))
...
if_else(is.null(nrow(smart)), message("No orders"),tablebuilder(impsmart))
```
...
if_else(is.null(nrow(biw)), message("No orders"),tablebuilder(impbiw))
```
...
if_else(is.null(nrow(pgg)), message("No orders"),tablebuilder(imppgg))
```
...
if_else(is.null(nrow(isl)), message("No orders"),tablebuilder(impisl))
```
...
if_else(is.null(nrow(iml)), message("No orders"),tablebuilder(impiml))
```
...
if_else(is.null(nrow(sime)), message("No orders"),tablebuilder(impsime))
```
...
#if_else(is.null(nrow(viridian)), message("No orders"),tablebuilder(impviridian))
```
...
if_else(is.null(nrow(mico)), message("No orders"),tablebuilder(impmico))
```

...
pathnamer<-paste("D:\\Orders to Import\\Import Files\\",filenamer,".xlsx")
```

```{r Autofilereader, include = FALSE, echo = FALSE}
# Read all files in the staging folder
farm<-list.files("D:\\Orders to import",pattern = "^FS CPSL Orders",full.names=TRUE)%>%lapply(.,read_xlsx)%>%do.call(rbind,.)
aciem<-list.files("D:\\Orders to import",pattern = "^Order_",full.names=TRUE)%>%lapply(.,read_xlsx, skip = 5)%>%do.call(rbind,.)
smart<-list.files("D:\\Orders to import",pattern = "^SL CPSL Orders",full.names=TRUE)%>%lapply(.,read_xlsx)%>%do.call(rbind,.)
biw<-list.files("D:\\Orders to import",pattern = "^BIPO",full.names=TRUE)%>%lapply(.,xl.read.file,password = "CPS2021")%>%do.call(rbind,.)%>%as_tibble()
pgg<-list.files("D:\\Orders to import",pattern = "^CPS \\d{2}",full.names=TRUE)%>%lapply(.,read_xlsx)%>%do.call(rbind,.)
isl1<-list.files("D:\\Orders to import",pattern = "^CPSL-Orders-from-ISL",full.names=TRUE)%>%lapply(.,read.csv)%>%do.call(rbind,.)%>%as_tibble()
isl2<-list.files("D:\\Orders to import",pattern = "^ISLPO\\d{6}",full.names=TRUE)%>%lapply(.,read.csv)%>%do.call(rbind,.)%>%as_tibble()
colnames(isl2) = c(names(isl1))
isl<-rbind(isl1,isl2)
iml<-list.files("D:\\Orders to import",pattern = "^IML GROUP",full.names=TRUE)%>%lapply(.,read_xlsx)%>%do.call(rbind,.)
sime<-list.files("D:\\Orders to import",pattern = "^\\d{7}", full.names = TRUE)%>%lapply(.,read_csv, show_col_types = FALSE)%>%do.call(rbind,.)%>%as_tibble()
viridian<-list.files("D:\\Orders to import",pattern = "^\\w{7,10} CPS Orders",full.names=TRUE)%>%lapply(.,read_xlsx)%>%do.call(rbind,.)
```

```{r Parameters, include = FALSE, echo = FALSE}
farmfiles<-list.files("D:\\Orders to import",pattern = "^FS CPSL Orders")
aciemfiles<-list.files("D:\\Orders to import",pattern = "^Order_")
smartfiles<-list.files("D:\\Orders to import",pattern = "^SL CPSL Orderst")
biwfiles<-list.files("D:\\Orders to import",pattern = "^BIPO")
pggfiles<-list.files("D:\\Orders to import",pattern = "^CPS \\d{2}")
isl1files<-list.files("D:\\Orders to import",pattern = "^CPSL-Orders-from-ISL")
isl2files<-list.files("D:\\Orders to import",pattern = "^ISLPO\\d{6}")
islfiles<-c(isl1files,isl2files)
imlfiles<-list.files("D:\\Orders to import",pattern = "^IML GROUP")
simefiles<-list.files("D:\\Orders to import",pattern = "^\\d{7}")
viridianfiles<-list.files("D:\\Orders to import",pattern = "^\\w{7,10} CPS Orders")
checklist<-list(farmfiles,aciemfiles,smartfiles,biwfiles,pggfiles,islfiles,imlfiles,simefiles,viridianfiles)
names(checklist)<-c("Farm_Source","Aciem","smart_Loyalty","BI_Worldwide","PGG","ISL","IML","Sime_Derby","Viridian")
processlist<-checklist
names(processlist)<-c("Farm_Source","Aciem","smart Loyalty","BI_Worldwide","PGG","ISL","IML","Sime_Derby","viridian")
processlist<-c(lapply(compact(checklist),length))
processlist<-data.frame(processlist, row.names = "Orders")
printlist<-compact(checklist)

filecount<-length(list.files("D:\\Orders to import", full.names = TRUE)%>%.[!file.info(.)$isdir])

message(paste(filecount,"orders read.","Please process the below customers"))
print(processlist)
```
...
deletions<-setdiff(names(checklist),names(processlist))
pattern<-c("Farm_Source","Aciem","smart_Loyalty","BI_Worldwide","PGG","ISL","IML","Sime_Derby","Viridian")
replacement<-c("impfarm","impaciem","impsmart","impbiw","imppgg","impisl","impiml","impsime","impviridian")
finallist<-mgsub(deletions, pattern = pattern, replacement = replacement)

deletions<-setdiff(names(checklist),names(processlist))
message("Hey Em, please DELETE the below customers from the printlist and then hit CTRL+Enter:    ")
print(finallist)

```

```{r Printlist}
printlist<-list(impfarm,impaciem,impbiw,imppgg,impisl,impiml,impsime,impviridian)

#Incase you need to reset the printlist:
#printlist<-list(impfarm,impaciem,impsmart,impbiw,imppgg,impisl,impiml,impsime,impviridian)
```
...
importcombined<-mutate(importcombined, 'Freight Total' =  importcombined$QTY * importcombined$`Freight`)%>%.[c(1:12,21,14:16)]
...
importcombined%>%write_xlsx(pathnamer)

```

```{r Archive raw files}
# Move the raw order files to an archive folder
archivetoday<-(paste0("D:\\Orders to import\\Archive\\",today()))
archivelist<-list.files("D:\\Orders to import", full.names = TRUE)%>%.[!file.info(.)$isdir]

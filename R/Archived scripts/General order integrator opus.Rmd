...
smart<-list.files("\\\\10.103.128.1\\PropellaEDI\\live\\Orders to import",pattern = "^SL CPSL Orders",full.names=TRUE)%>%lapply(.,read.csv)%>%do.call(rbind,.)%>%as_tibble()
...
sime<-list.files("\\\\10.103.128.1\\PropellaEDI\\live\\Orders to import",pattern = "^\\d{7}", full.names = TRUE)%>%lapply(.,read_csv, show_col_types = FALSE)%>%do.call(rbind,.)
...
simefiles<-sime<-list.files("\\\\10.103.128.1\\PropellaEDI\\live\\Orders to import",pattern = "^\\d{7}")
...
names(checklist)<-c("Farm Source","Aciem","smart Loyalty","BI Worldwide","PGG","ISL","IML","Sime Derby","viridian")
...
finalcheck<-list(impfarm,impaciem,impsmart,impbiw,impisl,impiml,impsime,impviridian)
importcombined<-compact(finalcheck)%>%reduce(full_join)
...
importcombined<-mutate(importcombined, 'Freight Total' =  importcombined$QTY * importcombined$`Freight Value`)%>%.[c(1:12,19,14:16)]

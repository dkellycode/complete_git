...
imagelist<-exif_read(directory, recursive = TRUE,tags = c("SourceFile","FileSize","FileTypeExtension","ImageWidth","ImageHeight"))%>%as_tibble()#%>%filter((FileTypeExtension in c("png","PNG","jpg","JPG","tff","TFF","tiff","TIFF")))
...
if(nrow(szexceptions)>0){for (i in seq_along(szexceptions$SourceFile)){szexceptions$SourceFile[i] %>% image_read() %>% image_scale(.,szexceptions$resizeval[i]) %>% image_write(.,szexceptions$SourceFile[i])}
rm(i)
gc()}
```
...
image_resize(picture, "x570")%>%image_border(., "transparent","97x0")%>%image_scale(., "650")%>%image_crop(., "650 x 570")%>%image_write(.,"/Users/d3ops/Documents/resizedcohesive.png")
